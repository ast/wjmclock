...
Linux x86_64 or aarch64 (Raspberry Pi 4/5). One line: downloads the right
pre-built binary into =~/wjmclock=, grabs the example config, checks that
the OpenGL / Wayland / X11 runtime libraries are present, and launches.
The script never uses ~sudo~ and never installs anything — if a library is
missing it prints the exact ~apt install~ command for you to run yourself.

#+begin_src bash
curl -fsSL https://raw.githubusercontent.com/ast/wjmclock/main/scripts/install.sh | bash
#+end_src

Read the script first if you'd rather:
[[file:scripts/install.sh][~scripts/install.sh~]] (~30 lines of logic, plain bash).
...
22.04+, Debian 12+, Pi OS bookworm+, RHEL 9+). Runtime deps are the same as
