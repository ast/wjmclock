...
Downloads the right pre-built binary into =~/wjmclock=, grabs the
...
never installs anything — if a library is missing it prints the ~apt
install~ command for you to run yourself.
